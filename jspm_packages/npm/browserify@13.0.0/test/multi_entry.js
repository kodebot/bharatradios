...
var fs = require('fs');
var testFiles = [__dirname + '/multi_entry/a.js', __dirname + '/multi_entry/b.js', __dirname + '/multi_entry/c.js'];
...
test('entries as streams', function(t) {
  ...
  b.on('dep', function(row) {
    if (row.entry) {
      t.similar(row.file, RegExp(__dirname + '/multi_entry/_stream_[\\d].js'), 'should be full entry path');
    }
  });
  b.bundle(function(err, src) {
    var c = {
