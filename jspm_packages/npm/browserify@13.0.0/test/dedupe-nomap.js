...
test('identical content gets deduped and the row gets a "nomap" flag set when sourcemaps are turned on', function(t) {
  t.plan(4);
  var rows = [];
  browserify({debug: true}).on('dep', [].push.bind(rows)).require(require.resolve('./dup'), {entry: true}).bundle(check);
...
test('identical content gets deduped and the row gets a "nomap" flag set when sourcemaps are turned off', function(t) {
  t.plan(4);
  var rows = [];
  browserify({debug: false}).on('dep', [].push.bind(rows)).require(require.resolve('./dup'), {entry: true}).bundle(check);
  function check(err, src) {
    if (err)
      return t.fail(err);
    var nomappeds = rows.filter(function(x) {
